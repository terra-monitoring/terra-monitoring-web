...
if ($table=='daily') {
    ...
    print '{"cols": [
                    {"label":"","type":"string"},
                    {"label":"Temperatur oben","type":"number"},
                    {"label":"Temperatur mitte","type":"number"},
                    {"label":"Temperatur unten","type":"number"}
                  ],';

    print '"rows": [';

    $result = $terra->query("SELECT * FROM '$query' WHERE time BETWEEN '$current_sunrise' AND '$current_sunset'");
